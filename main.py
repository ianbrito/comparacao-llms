...
def bertscore(texto_original, texto_gerado):
    scorer = BERTScorer(lang="pt_BR")
    scores = scorer.score([texto_gerado], [texto_original])
    return scores
...
def metricas(texto_gerado):
    ...
    doc_original = nlp(texto_orignal)
    ...
    cobertura = len(palavras_no_resumo.intersection(palavras_chave)) / len(palavras_chave)
    print(f"Cobertura de palavras-chave: {cobertura:.2%}")

    scores = rouge(texto_orignal, texto_gerado)

    print(f"ROUGE-1: {scores['rouge1'].fmeasure:.4f}")
    print(f"ROUGE-2: {scores['rouge2'].fmeasure:.4f}")
    print(f"ROUGE-L: {scores['rougeL'].fmeasure:.4f}")

    scores = bertscore(texto_orignal, texto_gerado)
...
def main():
    ...
    metricas(deepseek_texto_gerado)

    # gemma
    print("======================================= gemma =======================================")
    gemma_texto_gerado = gerar_resumo("gemma:latest", texto_orignal)
    
    print(gemma_texto_gerado)
    
    metricas(gemma_texto_gerado)

    # llama
    print("======================================= llama =======================================")
    llama_texto_gerado = gerar_resumo("llama3.2:3b", texto_orignal)

    print(llama_texto_gerado)
    
    metricas(llama_texto_gerado)

    # mistral
    print("======================================= mistral =======================================")
    mistral_texto_gerado = gerar_resumo("mistral:latest", texto_orignal)

    print(mistral_texto_gerado)

    metricas(mistral_texto_gerado)
